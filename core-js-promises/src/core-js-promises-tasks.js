...
/**
 * Takes an array of promises and
 * returns a promise that resolves with the value of the first successfully resolved promise From the given array.
...
/**
 * Attempts to resolve all provided promises.
 * If all promises resolve successfully,
 * it returns a promise that resolves with an array of their values.
 *
 * If any of the promises are rejected,
 * it returns a promise that is immediately
 * rejected with the reason of the first promise that was rejected.
 *
 * @param {Array<Promise<number>>} promises
...
function queuPromises(promises) {
  const enqueue = (index = 0, accumulator = '') => {
    if (index === promises.length) {
      return accumulator;
    }

    return promises[index].then((value) => {
      return enqueue(index + 1, `${accumulator}${value}`);
    });
  };

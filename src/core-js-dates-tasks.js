...
function isDateInPeriod(date, period) {
  const dateIn = Number(new Date(date));
  const startDate = Number(new Date(period.start));
  const endDate = Number(new Date(period.end));

  return startDate <= dateIn && dateIn <= endDate;
}
...
function formatDate(date) {
  const currLocaleDate = new Date(date);

  return currLocaleDate.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getCountWeekendsInMonth(month, year) {
  const currLocaleDate = new Date(year, month, 0);
  let days = currLocaleDate.getDate();

  let weekendsCount = 0;

  while (days > 0) {
    const currDay = currLocaleDate.getDay();
    ...
    days -= 1;
    currLocaleDate.setDate(currLocaleDate.getDate() - 1);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const sprint = countWorkDays + countOffDays;
  const schedule = [];
  let dayCounter = 0;

  while (startDate <= endDate) {
    if (dayCounter % sprint < countWorkDays) {
      schedule.push(toDateString(startDate));
    }

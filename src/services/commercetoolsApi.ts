...
async function createCustomer(userRegisterData: CreateUser, bearerToken: string): Promise<Customer> {
    const endpoint = `https://api.${apiRegion}.commercetools.com/${projectKey}/customers`;
...
export async function addDiscountCode(
    ...
    try {
        ...
        if (responseData.errors) {
            ...
            const badCodeError = responseData.errors.find(
                (error: ErrorObject) => error.code === "DiscountCodeNonApplicable"
            );

            if (badCodeError) {
                throw new Error(CT_BAD_PROMOCODE);
            }
